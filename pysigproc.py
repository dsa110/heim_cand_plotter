...
# Changed by Leon Houben to be more in line with PRESTO's filterbank.py
...
class SigprocFile(object):
    ...
    @property
    def tend(self):
        return self.tstart + self.nspec*self.tsamp/86400.0
    ...
    @property
    def bandpass(self):
        return self.get_data(nstart=0,nsamp=int(self.nspec))[:,0,:].mean(0)
    ...
    def get_data(self,nstart,nsamp):
        ...
        return numpy.frombuffer(self._mmdata[int(b0):int(b1)],
                dtype=self.dtype).reshape((-1,self.nchans)).T
